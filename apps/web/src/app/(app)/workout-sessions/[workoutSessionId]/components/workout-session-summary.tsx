...
export function WorkoutSessionSummary({ session }: { session: WorkoutSession }) {
  ...
  return (
    <div className="mx-auto grid max-w-3xl gap-3 p-2 md:p-4">
      <Card className="border-border/70 bg-card/80"><CardHeader className="items-center text-center"><div className="flex size-12 justify-self-center items-center justify-center rounded-full border border-primary text-primary">{isCompleted ? <Check className="size-7" strokeWidth={3} /> : <X className="size-7" strokeWidth={2.5} />}</div><CardTitle>{isCompleted ? "Great work!" : "Workout cancelled"}</CardTitle><CardDescription>{session.sourceRoutineNameSnapshot ?? "Freestyle workout"}</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-3"><div className="rounded-xl border border-border/70 p-3"><p className="text-xs text-muted-foreground">Exercises</p><p className="mt-1 text-xl font-semibold">{session.performances.length}</p></div><div className="rounded-xl border border-border/70 p-3"><p className="text-xs text-muted-foreground">Sets</p><p className="mt-1 text-xl font-semibold">{completedSets}</p></div><div className="rounded-xl border border-border/70 p-3"><p className="text-xs text-muted-foreground">Started</p><p className="mt-1 text-sm font-medium">{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(session.startedAt))}</p></div></CardContent></Card>
